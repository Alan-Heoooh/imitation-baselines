...
class RH100TDataset(Dataset):
    ...
    def _load_language_embeddings(self, task_descriptions):
        ...
        for task_name in tqdm(task_descriptions, desc="Embedding language conditions..."):
            ...
            task_descriptions[task_name]['embedding'] = np.array([])
            task_descriptions[task_name]['token_mask'] = np.array([])
        del clip_model
    ...
    def crop_resize_image(self, image, image_size=[180,320], crop_ratio=0.9):
        h, w = image_size
        h_new, w_new = int(h*crop_ratio), int(w*crop_ratio)
        resize = torchvision.transforms.Resize([h, w])
        if self.augmentation:
            crop = torchvision.transforms.RandomCrop([h_new, w_new])
        else:
            crop = torchvision.transforms.CenterCrop([h_new, w_new])
        image = resize(image)
        image = crop(image)
        image = resize(image)
        return image
    ...
    def __getitem__(self, index):
        ...
        for i, input_frame_id in enumerate(target_frame_ids[:self.num_input]):
            ...
            top_image = self.crop_resize_image(top_image)
            ...
            if self.wrist_cam_id is not None:
                ...
                wrist_image = self.crop_resize_image(wrist_image)

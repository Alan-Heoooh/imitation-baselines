...
def main(args):
    ...
    # dataset_root = '/aidata/chenxi/dataset/real_data_sampled'
    # dataset_root = '/aidata/jingjing/dataset/task303/80%realdata_sampled_20240426'
    # dataset_root = '/data/zihao_6024/data/wipe_general'
    dataset_root = args['dataset_root']

    task_config = ['RH100T_cfg1']
    task_descriptions = json.load(open('dataset/task_description.json'))
    train_dataset = RH100TReal(dataset_root, task_config, 'train', task_descriptions, num_input=1, horizon=chunk_size, timestep=-1, voxel_size=0.005, augmentation=True, num_sample='all', rot_6d=True, wrist_cam_id='cam_043322070878')
    ...
    val_dataset = RH100TReal(dataset_root, task_config, 'train', task_descriptions, num_input=1, horizon=chunk_size, timestep=-1, voxel_size=0.005, augmentation=False, rot_6d=True, wrist_cam_id='cam_043322070878')
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size_train, shuffle=False, num_workers=64, collate_fn=collate_fn, sampler=train_sampler)
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size_val, shuffle=False, num_workers=8, collate_fn=collate_fn, sampler=val_sampler)
...
def forward_pass(data, policy, device):
    obs_top = data['top_image_list']
    obs_wrist = data['wrist_image_list']
    action_data = data['target_frame_tcp_normalized']
    # convert to device
    obs_top, obs_wrist, action_data = obs_top.to(device), obs_wrist.to(device), action_data.to(device)
    return policy(obs_top, obs_wrist, action_data) # TODO remove None

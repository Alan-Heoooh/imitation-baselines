...
# from dataset.rh100t import RH100TDataset as RH100TPretrain
from dataset.rh100t_real import RH100TDataset as RH100TReal, collate_fn
from utils import compute_dict_mean, set_seed, detach_dict # helper functions
# import sys

# sys.path.append('/home/jingjing/suboptimal/robomimic/phase1/real_expert/train/imitation-baselines-main/act')
...
def main(args):
    ...
    train_dataset = RH100TReal(dataset_root, task_config, 'train', task_descriptions, num_input=1, horizon=1+chunk_size, timestep=-1, voxel_size=0.005, augmentation=True, num_sample='all', rot_6d=True, wrist_cam_id='cam_043322070878')
    # train_dataset = RH100TReal(dataset_root, task_config, 'train', task_descriptions, num_input=1, horizon=1+chunk_size, timestep=-1, augmentation=True, num_sample='10')
    # train_dataset = RH100TReal(dataset_root, task_config, 'train', task_descriptions, num_input=1, horizon=1+chunk_size, timestep=-1, augmentation=True, num_sample='39')
    val_dataset = RH100TReal(dataset_root, task_config, 'train', task_descriptions, num_input=1, horizon=1+chunk_size, timestep=-1, voxel_size=0.005, augmentation=False, rot_6d=True, wrist_cam_id='cam_043322070878')
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size_train, shuffle=False, num_workers=32, collate_fn=collate_fn, sampler=train_sampler)
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size_val, shuffle=False, num_workers=64, collate_fn=collate_fn, sampler=val_sampler)
...
def forward_pass(data, policy, device):
    top_image = data['top_image_list']
    wrist_image = data['wrist_image_list']
    qpos_data = data['input_frame_tcp_normalized']
    action_data = data['target_frame_tcp_normalized']
    is_pad = data['padding_mask']
    # convert to device
    top_image, wrist_image = top_image.to(device), wrist_image.to(device)
    qpos_data, action_data, is_pad = qpos_data.to(device), action_data.to(device), is_pad.to(device)
    return policy(qpos_data, top_image, wrist_image, action_data, is_pad) # TODO remove None
...
def synchronize_summary_dict(summary_dict, device, keys=['loss', 'l1', 'kl']):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_root', action='store', type=str, help='dataset path', default=None, required=True)
